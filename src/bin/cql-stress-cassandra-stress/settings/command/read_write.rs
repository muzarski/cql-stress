...
impl Parsable for Truncate {
    ...
    fn parse(truncate: &str) -> Result<Self::Parsed> {
        let create_err_msg = || {
            ...
            for v in Self::iter() {
                pattern.push(v.show().to_string());
            }

            format!(
                "Invalid truncate type: {}. Must match pattern: {}",
                truncate,
                pattern.join("|")
            )
        };
...
impl Parsable for ConsistencyLevel {
    ...
    fn parse(cl: &str) -> Result<Self::Parsed> {
        let create_err_msg = || {
            ...
            format!(
                "Invalid consistency level: {}. Must match pattern: {}",
                cl,
                pattern.join("|")
            )
        };
...
impl Parsable for SerialConsistencyLevel {
    ...
    fn parse(serial_cl: &str) -> Result<Self::Parsed> {
        let create_err_msg = || {
            ...
            format!(
                "Invalid serial consistency level: {}. Must match pattern: {}",
                serial_cl,
                pattern.join("|")
            )
        };
...
impl ReadWriteParams {
    pub fn print_settings(&self, command: &Command) {
        ...
        if self.duration.is_some() {
            println!("  Duration: {} SECONDS", self.duration.unwrap().as_secs());
        }
